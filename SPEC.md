...
- **2026-09-18, step 5 (meeting detail page).** No model calls were used.
  - **Action items are now editable.** Tick off, reassign from a menu (people in the meeting first, then the workspace), edit the text in place, add your own, remove. `POST | PATCH | DELETE /api/meetings/:id/action-items`. Items added by hand get `origin = 'manual'` and are labelled "added by you", so regenerating never touches them; extracted ones are labelled "from the meeting" and keep their timestamp link. A typed assignee is linked to the matching user when one exists. Updates are optimistic and rolled back if the server refuses. Every write is scoped by meeting id as well as item id: editing or deleting an item through another meeting's URL returns 404 and changes nothing (tested).
  - **Transcript.** Search with highlighted matches and a match count, filter by speaker with each speaker's share of the words spoken, "copy a link to this moment" on every line. Citation deep links still scroll to and highlight the exact line.
  - **Summary.** A meeting that has a transcript and no summary (the seeded ones, until their summaries are generated) shows a template picker and "Generate summary" instead of a dead end. "Copy as Markdown" puts the title, overview, sections with timestamps and the action item checklist on the clipboard.
  - **Header.** "Ask about this meeting" opens Ask scoped to it.
  - **Deferred to step 7.** Audio playback synced to the transcript, since there is no audio until upload and recording exist.
  - **Not exercised.** The successful path of the delete endpoint, because of the standing rule that nothing is deleted from the database without the owner seeing the rows first. Its rejection paths were tested.
- **2026-09-18, retrieval fix found on the live site: chunks are now embedded with their meeting title.** Asked live "What does Harbor Logistics need, and where do things stand with them?", Groq answered about a security questionnaire and missed the three must-haves. Everything it said was true and cited, so it passed faithfulness, and a prompt change ("use every relevant source") made no difference. The cause was retrieval: inside the passage where the customer lists her must-haves, nobody says "Harbor", so that chunk, embedded alone, was not retrieved for a question about Harbor. Chunks (and Scratchpad notes) are now embedded as `title + text`, and the keyword leg searches `tsv || to_tsvector(title)`. A meeting the model has to name is embedded after it has its title. All 32 existing chunks were re-embedded with `scripts/reembed.ts` (embedding calls and UPDATEs only). `KEEP` went from 10 to 12 so the latest meeting is not edged out at fusion. After: the same question returns all three must-haves, the nice-to-haves, the budget ceiling, beta status and the 120-seat letter of intent, citing 5 meetings in date order. The relevance floor still holds: off-topic 0.50, on-topic 0.63 to 0.73. Served by Groq `openai/gpt-oss-120b` throughout (5 requests).
  - **Still imperfect.** That answer says "October 20 launch": the passage that moves the date to October 27 is about the payment provider, so it is not retrieved for a question about Harbor. One earlier run conflated "beta agreement" with "grandfathering letters". An answer is only as current as what retrieval returns; there is no recency weighting yet.
- **2026-09-18, step 6 (meetings list, public share page, login).** No model calls.
  - **One summary, two views, enforced in code.** `components/summary-body.tsx` is the only renderer of a stored summary and is used by both the app and `/s/:slug`; only the link targets differ. The share page reads the current summary of the meeting's active template, the same row the owner sees. Tested: four probe sentences, including the three note-derived ones, appear identically in both views.
  - **What a share link exposes.** The summary, action items (read-only) and the transcript with search, speaker filter and deep links that stay on the public page. Never: the Scratchpad text, the "From your notes" marker, the template switcher, Ask, or any link into the app (0 found). Tested signed out. `noindex, nofollow`.
  - **Control.** Sharing is off by default. The Share dialog turns it on or off, copies the link, previews what others see, and can reset the link, which issues a new slug so the old URL stops working at once. Off, reset and never-existed are all the same 404. `POST /api/meetings/:id/share`, owner only (401 signed out).
  - **Meetings list.** Filter by title, person or summary text (with a hand-off to Ask when nothing matches), grouped by month, source icon (scheduled, instant, uploaded, pasted), who was there, action items done out of total, duration, date and time, a globe when the public link is on, and a status line: the summary overview, or "Transcript saved. No summary yet.", or writing, failed, or recording.
  - **Login.** Two columns: the pitch and one-click demo sign-in on the left, a static preview of a cited cross-meeting answer and a notes-corrected bullet on the right (real markup, not an image). A signed-in visitor is redirected away from it. The `from` parameter still refuses off-site redirects.
  - **State left in the workspace.** Sharing is ON for the kept AMI meeting so the owner can open its public page. Its summary is still the `gemini-3.5-flash-lite` one with the garbled bullet, pending tomorrow's regenerate.
- **2026-09-18, step 7 (audio upload, Whisper, in-browser recording, indicator, scheduled-meeting prompt).** All four parts built; the cut rule was not needed. Model calls: Whisper on Groq (its own quota) plus the normal summary and action item calls for 3 test recordings.
  - **The Blob store is private**, found on the first upload. Consequences, all handled: uploads use `access: "private"`; Groq cannot be given a URL, so the server reads the recording with the store token and sends Whisper the bytes; the browser cannot load the audio directly, so `GET /api/meetings/:id/audio` streams it to the owner only and passes `Range` through, which is what makes seeking work (tested: 200 full, 206 ranged, 401 signed out). Recordings are therefore never publicly readable, and the share page has no audio. The SDK must be given `token` explicitly: `vercel link` put a `VERCEL_OIDC_TOKEN` in `.env.local` and the SDK otherwise tries OIDC and fails locally.
  - **Audio to summary, measured** on a 26 second spoken clip (synthesized with macOS `say`): upload, Whisper `whisper-large-v3-turbo` 1.4 to 1.6 s, figures transcribed correctly ("November 1st", "$4,200"), then the usual pipeline. Served by: Whisper on Groq; summary by `gemini-3.5-flash-lite`; action items by `gemini-3.6-flash` on one run and Groq `gpt-oss-120b` on another. 12 of 12 citations exact. Only our own Blob URLs are accepted (a foreign URL is refused with 400), since the server fetches whatever it is given.
  - **Assignees without speaker labels.** Audio has no participants list, so the rule "assignee must be a participant" left every item unassigned. With no participants the name is now taken as spoken ("Jordan will book the movers" gives Jordan); "I will" stays unassigned because an unlabelled recording cannot say who "I" is.
  - **Recording.** `RecordingProvider` lives in the app layout, so a recording survives navigation. Lifecycle: `scheduled` to `recording` to `processing` to `ready`, or `failed`; the row exists from the first second, so the meetings list shows "Meetscribe is recording" too. Guards tested through the API: finish before start (409), start twice (409), foreign audio URL (400), cancel keeps the row and marks it failed. 25 MB guard: warns at 20 MB and stops itself just under the limit, keeping what it has. `beforeunload` warning while recording.
  - **The indicator**, the fix for the gap observed in Fathom: a dark pill at the top centre of every page with the Meetscribe mark, a pulsing red dot, "Meetscribe is recording", the meeting name, elapsed time and Stop. The tab title becomes "● REC 0:05" and the favicon gets a red dot, so it is visible from another tab. There is no code path that records without it. Instant and scheduled meetings use the same path and the same indicator.
  - **Scheduled-meeting prompt.** Calendar sync is stubbed, as planned: "Simulate a calendar meeting in 2 min" in the sidebar creates a `scheduled` row, which appears under Upcoming and raises a branded prompt ("Meeting starting in 2m", "Join & capture audio") from 10 minutes before until 30 minutes after the start time.
  - **Verified in a real browser.** Headless Chrome with a fake microphone drove the UI: start, indicator present, row in the meetings list, in-app navigation to Ask with the indicator and "● REC" title still there, favicon swapped, Stop, "Saving your recording…", upload, redirect, title and icon restored. Two bugs found that way and fixed: the favicon swap was undone by the framework re-inserting its icon on navigation (now repointed on every tick); the pill first sat bottom centre, on top of the Ask input (moved to top centre).
  - **Silence.** Chrome's fake microphone never played the supplied WAV, so those recordings were silent, and Whisper turned 30 seconds of silence into one segment, "Thank you.", which was then summarised as a meeting. Groq reported `no_speech_prob = 0` for it, so that field alone does not catch it. Rule added: 4 words or fewer across 15 seconds or more is treated as no speech, and the meeting fails with "No speech was found in this recording. Check that the right microphone was selected."
  - **NOT verified: real speech through a real microphone.** Everything around it is (the recorder, upload, lifecycle, and Whisper on real speech via file upload), but a human needs to record a sentence once in a real browser.
  - **Audio player.** On the Transcript tab when a recording exists: clicking a line's time plays from there, the line being spoken is highlighted, and a citation deep link positions the player at that moment.
- **2026-09-18, evening: submission tonight, so the remaining validation plan was cut.** The owner could not wait for Gemini's reset.
  - **Disclosed limitation, not a hidden gap.** Two checks were validated with a lighter or alternate model because the free-tier quota of the model that meets the quality bar was exhausted, and they were **not** repeated on a full Gemini Flash model: (1) regenerating a summary from Scratchpad notes, verified on `gemini-3.5-flash-lite` and on Groq `gpt-oss-120b`; (2) the fall-through for a transcript too long for Groq, where the mechanism is verified (Groq skipped with no request, chain traversed in order) but no provider had quota left to produce the final summary. The 33 minute real AMI meeting `ES2011d`, fetched later, is the right input for that re-test. The README says the same.
  - **Gemini's quota is not a simple daily reset.** At 17:33 UTC a one-line request to 3.6, 3.5 and 3.8 returned 200, and a transcript-sized request seconds later returned 429 on all three. Either tiny requests slip under a token-based limit or the window is rolling. It was not investigated further.
  - **Every demo summary was written by Groq `openai/gpt-oss-120b`**: the kept AMI meeting and all seven seed meetings. Short meetings now get reasoning effort "medium" on Groq, chosen automatically when prompt plus a 2,400-token completion fits under 6,500 tokens; longer ones stay on "low".
  - **Each summary was read against its transcript.** No errors found in: Dispatch Launch Kickoff, Harbor Logistics Discovery Call, Engineering Sync. Errors found: a board-approval threshold reported as $69 where the speaker corrects himself to $50, and "would only approve $40" for "approved without a fight" (Pricing Review); a month invented for a bare day number, "October 22" and "October 21", where the speakers said "the twenty second" and "Monday the twenty first" (Launch Readiness, Design Review); a statement attributed to the person who asked the question (Standup); "as stated in the slide", which nobody said (AMI). Prompt rules were added for each pattern and the five were regenerated: Design Review, Standup and AMI came back correct, the "only" distortion went away, and the AMI budget figure is now quoted as spoken ("twelve point twelve fifty") instead of being tidied into "12.125".
  - **Two errors survived the rules**, so they were fixed the way the product intends a person to fix them: a Scratchpad correction, then regenerate. Pricing Review now says board approval applies over $50 "(corrected from $69)" and carries the "From your notes" badge. For Launch Readiness the regeneration returned a summary with every section empty; the previous version was restored (superseded versions are kept, so this was an UPDATE). **That summary still says the security questionnaire is due "October 22"; the correct date, September 22, is in its Scratchpad and on its action item**, and the page shows the "notes changed" banner. Its action items were reduced from 6 to 2 by that failed regeneration.
  - **Groq has a daily token cap as well: 200,000 per model.** Today's testing and these summaries used 196,000 on `gpt-oss-120b`. It refills on a rolling basis. Because Ask runs on the same model, `openai/gpt-oss-20b` (its own 200,000) was added behind it: tested on the cross-meeting launch-date question, correct, cited, 5 s. It cannot produce the strict JSON summaries need, so for summaries the chain simply moves past it. The Groq call now waits and retries in a loop within `GROQ_MAX_WAIT_MS` instead of once, after two parallel calls for one meeting repeatedly exceeded the per-minute budget.
  - **Step 9 (polish).** Loading skeletons on navigation, an error boundary with Try again, a phone layout (sidebar becomes a top bar below 768 px), a short settle animation with `prefers-reduced-motion` respected, thin scrollbars, Open Graph metadata, and a correct status line for cancelled or silent recordings. `framer-motion` was removed, unused.
  - **Step 10.** `README.md` and `WALKTHROUGH.md`.
- **2026-09-18, late: real authentication added (section 1 said "Demo auth"; this supersedes it).** GitHub OAuth through Auth.js v5, JWT session strategy, so there is no session or token table and **no migration**: on first sign-in a row is inserted into the existing `users` table and its id rides in the encrypted session cookie as `uid`. `getCurrentUser()` is still the single place a request becomes a user; it now accepts a GitHub session first and the demo cookie second, so every ownership check downstream is unchanged. The proxy lets either session through and leaves `/api/auth/*` open.
  - **Why GitHub and not an emailed magic link**, which was the first idea: without a verified sending domain Resend only delivers to the account owner, so reviewers could not have signed in; a magic link also needs an adapter and new tables, and preview builds run migrations against the same database production uses. Reviewers of a GitHub repository have GitHub accounts.
  - **The demo workspace stays**, as a second button. A new account's workspace is empty, because every seeded meeting belongs to the demo user, and a reviewer should not have to create data to see the product. It is also the fallback if OAuth is ever misconfigured.
  - **Verified locally against a production build.** Signed out: app pages redirect to login with a return path, `/api/ask`, the audio stream and the share toggle return 401, the share page and `/api/auth/providers` are public. GitHub button: 303 to `github.com/login/oauth/authorize` with `read:user user:email`, PKCE and the right callback. Demo button: unchanged. With a genuine Auth.js session (minted with the server's own secret, since GitHub's consent screen cannot be scripted): own workspace is empty, the demo user's meeting renders the not-found page with none of its content in the HTML, its audio and share endpoints return 404, and Ask retrieves 0 excerpts from the demo data. A tampered Auth.js cookie, one signed with a different secret and forged as the demo user, and a tampered demo cookie are all rejected. `from=//evil.example` returns to `/meetings` on both buttons. Sign-out clears both cookies. A signed-in visitor to `/login` is redirected.
  - **Known cosmetic point.** Another user's meeting returns the not-found page with HTTP 200, not 404: the loading skeleton added in step 9 starts the response before the ownership check finishes. No content is exposed.
  - **Not verified by me:** the GitHub consent click-through itself, which the owner does by hand on the preview deployment.
- **2026-09-19: landing page, and "Record a meeting tab".**
  - **Landing page at `/`**, public; a signed-in visitor still goes straight to `/meetings`. It follows the structure of Fathom's public page (hero, product proof, three pillars, inputs, closing call to action) in Meetscribe's own identity, with product previews in real markup. It says plainly that no bot joins calls and that this is an assignment rebuild, not affiliated with Fathom. The owner also asked for the whole app to look like Fathom; that was **not** done: the logged-in Fathom app was never seen (only described), the original decision was "explicitly not Fathom's purple" for a UI-judged brief, and a full restyle hours before submission would touch every verified screen. It is an open decision.
  - **Record a meeting tab**, the closest real thing to a bot on a free tier. The user joins the call in a browser tab; `getDisplayMedia` opens the browser's own picker, the user chooses that tab and ticks "Also share tab audio"; the tab's audio (everyone else) is mixed with the microphone (the user) through Web Audio into one stream, which goes down the existing recorder, indicator, upload and Whisper path unchanged. Only audio is recorded; the video track the picker insists on is kept alive and never stored. Ending the share from the browser's own bar stops the recording cleanly. Guards: no `getDisplayMedia` (Safari, Firefox, phones), picker cancelled, and the common mistake of sharing a window or leaving the audio box unticked, each with a specific message; the microphone is optional. Chrome or Edge on a computer only.
  - **Verified:** in headless Chrome with the picker auto-selecting a second tab and a trusted click (a scripted click is refused by `getDisplayMedia`): capture started with a tab audio track and the microphone, the indicator read "Meeting tab · meeting tab + microphone", 33 s recorded, 133 KB uploaded (a silent recording of that length is about 7 KB), saved, redirected. The microphone path was re-run after the refactor and still passes end to end.
  - **NOT verified: that it hears a real call.** Whisper found no speech in that headless recording, and it cannot be told from here whether a headless tab emits its audio at all. A person has to try it once with a real Google Meet.
  - **Owner confirmed by hand (2026-09-19):** GitHub sign-in on production after the callback URL change, a real microphone recording, and the Share dialog.
- **2026-09-19: restyled after Fathom's app (supersedes "Design direction" in section 3).** The owner asked for the UI to look like Fathom. The logged-in app could not be seen, but Fathom's public site carries its own product screenshots, and those were used as the reference: a near-black canvas with cards one step lighter, a cyan-blue accent (not the violet of their marketing page), and a two-column meeting page with the chat docked on the right.
  - **Theme.** Done almost entirely by redefining the colour tokens in `globals.css`, which is what they were for: canvas, surfaces, lines, four ink levels, accent, recording red, warning, notes violet, shadows, plus `color-scheme: dark` so the native audio player and form controls follow. A new `on-accent` token carries text that sits on the accent, because white on this cyan fails contrast and near-black passes. Hard-coded spots fixed by hand: the primary button, speaker colours (now light tints, used as text on dark and as avatar fills with dark initials), dialog overlays, the brand mark and favicons. The recording indicator became a **light pill on the dark UI**, so it stands out more than before; red is still used for nothing but recording.
  - **Layout.** On screens 1280 px and wider the meeting page has Ask docked in a right-hand column, scoped to that meeting with a pill that widens it to all meetings, as in Fathom. It is the same `AskChat` component with a `compact` flag, not a second implementation. Below that width the header keeps the "Ask about this meeting" button.
  - **What was deliberately not copied:** Fathom's name, logo, wording, illustrations and photographs. The landing page states that this is an assignment rebuild and is not affiliated with Fathom.
  - **Checked:** typecheck, lint, 58 tests, a production build, and screenshots of the meeting page, meetings list, landing page and login page. No behaviour changed.
- **2026-09-19: the recording indicator now follows the user across tabs and apps; a stuck recording can be ended.**
  - **Reported by the owner:** a recording that could not be stopped, and the expectation that the Stop control should be visible after switching to the meeting's own tab.
  - **Stuck recording.** No fault was found in Stop. The likely cause was a page reload during the recording (a deploy landed mid-recording), which destroys the recorder and releases the microphone while the row stays `recording`, with no control anywhere to end it. The meeting page now distinguishes a recording running in this tab ("press Stop in the indicator") from one that is not ("End this recording", which marks it failed with the reason; the audio of a closed tab cannot be recovered). Verified as the meeting's real owner on production.
  - **Indicator across tabs.** A web page cannot draw inside another site's tab, so the pill cannot appear inside Google Meet. It now also opens in a **Document Picture-in-Picture window**: a small always-on-top window above every tab and every other application, showing the same indicator, a live timer and a working Stop. It is rendered by a React portal from the same component tree, with the app's stylesheets copied in, so it is the same indicator, not a copy. The browser only grants that window within moments of a click, and the tab picker can take much longer, so it is requested synchronously at the start of `start()`, before the first `await`; measured in Chrome, the window request and `getDisplayMedia` both succeed from one click, in either order. If the user closes the floating window the recording continues and the in-app pill gains a button to float it again. Chrome and Edge on a computer; elsewhere nothing changes. In tab mode Chrome's own "Stop sharing" bar on the meeting tab was already a second way to stop.
  - **Verified in a visible Chrome window** (headless Chrome has no such API): started with a trusted click, floating window open and styled, timer ticking inside it, Stop pressed inside the floating window, recording saved, app redirected, floating window closed itself. `scripts/floating-indicator-e2e.mjs`.
